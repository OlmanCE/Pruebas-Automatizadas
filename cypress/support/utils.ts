...
const generateLongContent = (length: number) => {
  return '1234567890'.repeat(length);
};
...
export const createNewTopic = (title: string, content: string, categories: string[]) => {
  ...
  if (title) {
    if (title === "212") {
      title = generateLongContent(212) + '1';
      cy.get('input[id="reply-title"]')
        .invoke('val', title)
        .trigger('input');
    } else {
  ...
  if (content) {
    if (content === "32000") {
      content = generateLongContent(3200) + '1';
      cy.get('textarea.d-editor-input')
        .invoke('val', content)
        .trigger('input');
    } else {
      cy.get('textarea.d-editor-input').type(content);
    }
  }
